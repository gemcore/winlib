...
int TED_Thread::edit(void)
{
   ...
   eflag = 0;      // filename prompt when saving
   xflag = 0;      // exit editor
   term_putstatus4(sflag);
   ...
   while (1)
   {
      ...
      *(p+k) = '\0';
      ...
      if (c == EDIT_CTL_E)         // KC_CTL_E
      {
         ...
         // Set flag to prompt for filename..
         eflag = 1;
         return 0;
      }
   ...
   while (ted_pt->active)
   {
      ...
      if (ted_pt->sflag || ted_pt->eflag)
      {
         if (ted_pt->xflag)
         {
            ...
            {
               term_putprompt("Buffer is unchanged. Exit [yn]?");
            }
