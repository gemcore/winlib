...
extern void MEM_Dump(uint8_t *data,uint16_t len,uint16_t base);
...
void LexCleanup(Picoc *pc)
{
    ...
    for (Count = 0; Count < sizeof(ReservedWords) / sizeof(struct ReservedWord); Count++) {
        char *p = TableStrRegister(pc, ReservedWords[Count].Word);
        TableDelete(pc, &pc->ReservedWordTable, p);
    }
}
...
void *LexTokenize(Picoc *pc, struct LexState *Lexer, int *TokenLen)
{
    ...
    do {
        ...
        TRACE(" %3d: %02x={%s}", MemUsed, Token, LexTokenToStr(Token));
	...
	{
	    MEM_Dump((unsigned char*)HeapMem, MemUsed, 0x0000);
    }
#endif    
    if (TokenLen) {
        *TokenLen = MemUsed;
    ...
    int IncPos)
{
    ...
    TRACE("Got token inc=%d pos=%3d %02x={%s}\n", IncPos, Parser->CharacterPos, Token, LexTokenToStr(Token));
...
#if HAS_TRC == 1
void LexBaseTypeToStr(enum BaseType type)
{
    char* BaseNames[] = {
        /* 0x00 */  "Void",
        /* 0x01 */  "Int",
...
char *LexTokenToStr(enum LexToken Token)
{
    char* TokenNames[] = {
        /* 0x00 */  "None",
        /* 0x01 */  "Comma",
