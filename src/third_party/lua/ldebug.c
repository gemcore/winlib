...
void luaG_typeerror (lua_State *L, const TValue *o, const char *op) {
  ...
  const char *kind = (isinstack(L->ci, o)) ?
                         ...
                         NULL;
  TRACE("luaG_typeerror: op=%s kind=%s name=%s t=%s\n",op,kind,name,t);
  if (kind)
...
static void addinfo (lua_State *L, const char *msg) {
  ...
  if (isLua(ci)) {  /* is Lua code? */
    ...
    //ADG!luaO_pushfstring(L, "%s:%d: %s", buff, line, msg);
    int i=strlen(buff);
    sprintf(&buff[i],": %d:",line);
...
void luaG_runerror (lua_State *L, const char *fmt, ...) {
  va_list argp;
  va_start(argp, fmt);
  CON_printf("luaG_runerror: ");
  CON_vprintf(fmt, argp);	//ADG!
  CON_printf("\n");
  CON_Flush();
