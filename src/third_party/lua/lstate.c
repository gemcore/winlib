...
//ADG!#include "elua_int.h"
#include "platform.h"
#endif
// BogdanM: linenoise cleanup
#include "linenoise.h"
...
LUA_API lua_State *lua_newstate (lua_Alloc f, void *ud) {
  ...
  TRACE("lua_newstate() calling luaD_rawrunprotected: f_luaopen=%x ...\n",f_luaopen);
...
{
  if (status == PLATFORM_CPU_DISABLE)
  {
    // Disable interrupts.
    return MAP_IntMasterDisable();	// If interrupts were disabled then return value will be 0
  }
  else
  {
    // Enable interrupts.
    return MAP_IntMasterEnable();	// if interrupts were enabled then return value will be 1
  }
}
...
LUA_API void lua_close (lua_State *L) {
  ...
  // If interrupts were enabled when we turned them off, so turn them back on again.
  if (!ui32Int)
  {
      MAP_IntMasterEnable();
  }
  ...
  do {  /* repeat until no more errors */
    ...
    L->nCcalls = L->baseCcalls = 0;
  TRACE("lua_close() calling luaD_rawrunprotected: callallgcTM=%x ...\n",callallgcTM);
