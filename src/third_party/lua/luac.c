...
{
 fio_printf(stderr,"%s: cannot %s %s: %s\n",progname,what,output,strerror(errno));
 fio_flush(stderr);
 TRACE("cannot(%d)\n",EXIT_FAILURE);
 exit(EXIT_FAILURE);
}
...
{
 if (*message=='-')
   fio_printf(stderr,"%s: unrecognized option " LUA_QS "\n",progname,message);
 else
   fio_printf(stderr,"%s: %s\n",progname,message);
 fio_printf(stderr,
...
{
 UNUSED(L);
 return (fio_write((FIO_FILE*)u,p,size,1)!=1) && (size!=0);
}
...
static int pmain(lua_State* L)
{
 ...
 if (dumping)
 {
  FIO_FILE *D= (output==NULL) ? stdout : fio_open(output,"wb");
  ...
  if (fio_error(D)) cannot("write");
  if (fio_close(D)) cannot("close");
 }
 return 0;
}
