...
extern int RingBufPeek(tRingBufObject *psRingBuf)
{
    return RingBufGetAt(psRingBuf, 0UL);
}

extern int RingBufGetAt(tRingBufObject *psRingBuf, uint32_t ui32Index)
{
#ifndef WIN32
	bool bIntsOff;
#endif
    ...
    //
    // Peek ahead at the buffer byte that is at a specified read index. 
    // Note that we must do this within a critical section (interrupts 
    // disabled) to prevent incorrect access to the read buffer indices.
    //
#ifndef WIN32
	bIntsOff = IntMasterDisable();
#endif
    ui32Used = RingBufUsed(psRingBuf);
    if (ui32Used >= ui32Index)
    {
        uint32_t ui32Offset = RingBufContigUsed(psRingBuf);
        uint32_t ui32Read = psRingBuf->ui32ReadIndex;
        if (ui32Index >= ui32Offset)
        {
            ui32Index -= ui32Offset;
        }
        b = psRingBuf->pui8Buf[ui32Read + ui32Index] & 0xFF;
    }
