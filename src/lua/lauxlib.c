...
static const char *getF (lua_State *L, void *ud, size_t *size) {
  ...
  if (lf->srcp == NULL) { // no direct access
    if (fio_eof(lf->f)) return NULL;
    *size = fio_read(lf->f, lf->buff, 1, sizeof(lf->buff));
...
LUALIB_API int luaL_loadfile (lua_State *L, const char *filename) {
  ...
  readstatus = fio_error(lf.f);
  if (filename) fio_close(lf.f);  /* close file (even in case of errors) */
  if (readstatus) {
...
static void *l_alloc (void *ud, void *ptr, size_t osize, size_t nsize) {
  ...
  TRACE("l_alloc ptr=%x osize=%d nsize=%d\n",ptr,osize,nsize);
  //lua_assert(ptr != NULL);
  ...
  if(nsize > osize && L != NULL) {
    ...
    if(G(L)->memlimit > 0 && (mode & EGC_ON_MEM_LIMIT) && l_check_memlimit(L, nsize - osize)) {
      TRACE("l_alloc() nsize=%d memlimit=%d reached!\n",nsize,G(L)->memlimit);
      return NULL;
    }
  }
  ...
  if (nptr)
  {
    TRACE(" ALLOC ptr=%08x nsize=%3d nptr=%08x\n",ptr,nsize,nptr);
  }
  else
  {
      TRACE("l_alloc() failure!\n");
  }
  return nptr;
}
...
static int panic (lua_State *L) {
  ...
  CON_printf("PANIC: unprotected error in call to Lua API (%s)\n",lua_tostring(L, -1));
  CON_Flush();
  return 0;
}
