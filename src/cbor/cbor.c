    ...
    switch(majorType) {
        ...
        case 3: // text
            if(minorType < 24) {
                ...
                token->type = CBOR_TOKEN_TYPE_ERROR;
                token->error_value = "text too long";
                return offset;
            } else {
                token->type = CBOR_TOKEN_TYPE_ERROR;
                token->error_value = "invalid text length";
                return offset;
            }
            break;
        case 4: // array
            if (minorType == 31) { // * array
        ...
        case 7: // special
            if (minorType == 31) {
                token->type = CBOR_TOKEN_TYPE_BREAK;
                token->int_value = minorType;
                return current_offset;
            } else if (minorType < 24) {
    ...
    switch(majorType) {
        ...
        case 3: // text
            if(remaining < token->int_value) {
                ...
                token->type = CBOR_TOKEN_TYPE_TEXT;
                token->text_value = (char *) (data + current_offset);
                return current_offset + token->int_value;
            }
        case 4: // array
        case 5: // map
